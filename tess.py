...
def parser1(d):
    ...
    if not bool(len(yess)):
        ...
        splits[0] + ' ' + splits[1] + ' ' + splits[2] + " " + splits[3], splits[3].replace('#', ''), splits[4],
        owner_nft,
        table5.find_elements(By.XPATH, "//div[contains(@class, 'NFTCard__content___28_Xp')]/img")[d].get_attribute(
...
#  driver = webdriver.Chrome(service = service1, options=options)
  ...
  for d in np.arange(0, 1470):
    if (d % 6) == 0 :
        t1 = Thread(target=parser1(d))
        t1.start()
    elif(d % 6) == 1:
        t2 = Thread(target=parser2(d))
        t2.start()
    elif (d % 6) == 2:
        t3 = Thread(target=parser3(d))
        t3.start()
    elif (d % 6) == 3:
        t4 = Thread(target=parser4(d))
        t4.start()
    elif (d % 6) == 4:
        t5 = Thread(target=parser5(d))
        t5.start()
    elif (d % 6) == 5:
        t6 = Thread(target=parser6(d))
        t6.start()
